 ...
 */
export function createState<T>(key: string, defaultValue?: T): TypedState<T> {
  if (defaultValue !== undefined && !Registry.hasState(key)) {
    Registry.setState(key, defaultValue);
  }

  return {
    key,
    get(): T | undefined {
      return Registry.getState(key) as T | undefined;
    },
    set(value: T): void {
      Registry.setState(key, value);
    },
    subscribe(callback: (newValue: T | undefined, oldValue: T | undefined) => void): () => void {
      return Registry.subscribe(key, callback as (newValue: unknown, oldValue: unknown) => void);
    },
    has(): boolean {
      return Registry.hasState(key);
    },
    delete(): void {
      Registry.deleteState(key);
    }
  };
}
